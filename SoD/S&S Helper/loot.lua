...
local QuestItemToQuestId = {
    [9250] = 2876, -- Ship Schedule
    [2794] = 337, -- Old History Book
    ...
    [23798] = 9535, -- Diabolical Plans (Horde only)
    [23780] = 9520, -- Diabolical Plans (Both?)
	...
	[12558] = 4882, -- Guarding Secrets
	[23338] = 9373, -- Eroded Leather Case
	[28552] = 10229, -- Decipher the Tome
...
function SSHelper:PLAYER_INTERACTION_MANAGER_FRAME_SHOW(paneType)
...
function SSHelper:PLAYER_INTERACTION_MANAGER_FRAME_HIDE(paneType)
    if paneType ==  Enum.PlayerInteractionType.TradePartner then
        isLooting = false
    end
    if paneType ==  Enum.PlayerInteractionType.Merchant then
    ...
    SSHelper:AutomaticOpener()
    SSHelper:QuestStarterFromItem()
end

function SSHelper:LOOT_OPENED()
    self:DebugLog("LOOT_OPENED")
    isLooting = true
end

function SSHelper:LOOT_CLOSED()
    self:DebugLog("LOOT_CLOSED")
    isLooting = false
    SSHelper:AutomaticOpener()
    SSHelper:QuestStarterFromItem()
end

function SSHelper:PLAYER_REGEN_DISABLED()
    self:DebugLog("PLAYER_REGEN_DISABLED")
    inCombat = true
end

function SSHelper:PLAYER_REGEN_ENABLED()
    self:DebugLog("PLAYER_REGEN_ENABLED")
    inCombat = false
    SSHelper:AutomaticOpener()
    SSHelper:QuestStarterFromItem()
end

function SSHelper:AutomaticOpener()
    if not SS.db.profile.autoUseItem then
        self:DebugLog("AutomaticOpener: Disabled")
        return
    end
    self:DebugLog("AutomaticOpener")
    if (atBank or atMail or atMerchant or inCombat or isLooting or isCrafting) then
        self:DebugLog("AutomaticOpener: Not opening")
        return;
    end
    ...
    for bag = 0, 4 do
        for slot = 0, C_Container.GetContainerNumSlots(bag) do
            ...
            if id and AllowedItemsList[id] then
                ...
                DEFAULT_CHAT_FRAME:AddMessage("|cff00FF80SSHelper : Opening " .. C_Container.GetContainerItemLink(bag, slot))
...
function SSHelper:QuestStarterFromItem()
    if not SS.db.profile.autoUseItem then
        self:DebugLog("QuestStarterFromItem: Disabled")
        return
    end
    self:DebugLog("QuestStarterFromItem")
    if (atBank or atMail or atMerchant or inCombat or isLooting or isCrafting) then
        self:DebugLog("QuestStarterFromItem: Not opening")
        return;
    end
    ...
    for bag = 0, 4 do
        for slot = 0, C_Container.GetContainerNumSlots(bag) do
            local id = C_Container.GetContainerItemID(bag, slot)
            if id and QuestItemToQuestId[id] then
                for _ ,quest in ipairs(SS.quests) do
                    if (quest.id == QuestItemToQuestId[id]) then
                        self:DebugLog("Already have the quest: " .. quest.title)
                        return
                    end
                end
                if C_Container.GetContainerItemInfo(bag, slot).isLocked then return end
