...
function SS:convertOrderedTalentIndexToWowIndex(classNameToLookup, talentSpecNumber, orderedTalentIndex)
	local data = SS.indexToWowIndex[classNameToLookup][talentSpecNumber][orderedTalentIndex]
	return data
end

function SS:convertTalentSpecNameToWowIndex(class, spec)
    local data = talentPages[class][spec]
    return data
end
...
local talents = {
    DEATHKNIGHT = { -- Unholy
        {Unholy = {2,3,0,0,3,0,3,0,5,0,0,3,2,1,5,2,0,0,0,1,5,0,0,1,3,1,3,3,0,5,1}},
        {Blood = {}},
        {Frost = {3,2,0,0,4,3,5,0,0,0,0,2}},
    },
    DRUID = { -- Feral Cat
        {Feral = {5,5,0,2,0,2,1,3,0,3,2,2,0,1,2,0,5,3,1,2,0,0,3,3,3,1,3,0,0,1}},
        {Balance = {}},
        {Restoration = {2,0,5,5,0,3,0,1,2}},
    },
    HUNTER = { -- BeastMastery
        {BeastMastery = {5,1,2,0,0,2,0,1,5,0,5,2,1,2,1,4,3,1,0,2,5,2,1,0,5,1}},
        {Marksmanship = {0,0,5,0,0,5,2,1,0,2,0,5}},
        {Survival = {}},
    },
    MAGE = { -- Frost
        {Frost = {0,2,3,3,0,0,3,3,1,3,2,3,3,1,0,0,2,3,0,1,5,2,2,3,1,0,5,1}},
        {Arcane = {2,3,0,3,0,5,0,3,0,0,0,0,3}},
        {Fire = {}},
    },
    PALADIN = { -- Retribution
        {Retribution = {0,5,2,3,1,2,5,1,2,0,3,3,3,1,3,2,2,1,3,3,0,0,1,3,3,1}},
        {Holy = {0,5,3,2,0,1}},
        {Protection = {0,5,0,2}},
    },
    PRIEST =  { -- Shadow
        {Shadow = {3,2,5,0,2,3,0,5,1,0,2,3,0,1,2,3,2,3,1,4,2,3,1,1,3,5,1}},
        {Discipline = {0,5,0,3,2,0,3}},
        {Holy = {}},
    },
	ROGUE = { -- Combat
        {Combat = {0,2,5,2,0,5,1,0,5,0,0,3,5,0,1,0,2,2,3,1,0,0,5,0,1,2,5,1}},
        {Assassination = {0,0,5,3,2,0,0,0,5,1,4}},
		{Subtlety = {}}
	},
    SHAMAN = { -- Enhancement
        {Enhancement = {3,0,2,0,5,0,1,3,1,0,5,0,2,1,3,3,3,0,3,1,1,2,1,2,3,1,0,5,1}},
        {Elemental = {0,5,3,0,3,0,1,5,2}},
        {Restoration = {}},
    },
    WARLOCK = { -- Affliction
        {Affliction = {2,3,5,0,0,2,2,0,3,1,2,0,3,5,1,0,2,5,3,5,0,2,3,3,1,1,5,1}},
        {Demonology = {0,0,3,2,2,0,3,0,0,0,1}},
        {Destruction = {}}
    },
    WARRIOR = { -- Arms
        {Arms = {3,3,2,2,3,0,2,0,2,3,3,3,5,1,0,0,2,0,1,0,1,2,2,1,0,2,3,1,2,5,1}},
        {Fury = {3,0,5}},
        {Protection = {2,0,3,3}}
    }
}

function SS:LearnTalents()
    local class = self.db.char.class
    local autoLearn = self.db.profile.autoTalent

    if not autoLearn then
        return
    end
    print("Learning Talents")
	for _, spec in pairs(talents[class]) do
		for specName, talentTableValues in pairs(spec) do
            for specKey, talentTableValue in pairs(talentTableValues) do
                if talentTableValue > 0 then
                    local specIndex = SS:convertTalentSpecNameToWowIndex(class, specName)
                    local mappedIndex = SS:convertOrderedTalentIndexToWowIndex(class, specIndex, specKey)
                    for _ = 1, talentTableValue do
    ---@diagnostic disable-next-line: redundant-parameter
                        -- self:DebugLog("Learning Talent: " ..  .. " " .. mappedIndex)
                        LearnTalent(specIndex, mappedIndex, false)
                    end
                end
			end
		end
